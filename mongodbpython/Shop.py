...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.productChosen['values'] = (
            "Chips", "Choclates", "Drinks", "Fruits", "Pies", "Sweets", "Veggies")
        self.productChosen.current(0)
        self.brandChosen = ttk.Combobox(self, textvariable=16)
        self.brandChosen['values'] = ("Simba", "Lays", "No_Brand", "BarOne", "Chomp", "Coca-Cola", "Fanta",
                                      "Flake", "Maynards", "Monster Energy", "Play", "PS", "Score", "Speckles", "Sprite", "Tex")
        self.brandChosen.current(0)
        self.btChips = tk.Button(
            self, command=self.categoryClick, text="Display Category", height=2, width=14)
        self.btnRestore = tk.Button(
            self, command=self.restoreDB, text="Restore Database", height=2, width=14)
        self.scr = scrolledtext.ScrolledText(
            self, width=75, height=10, wrap=tk.WORD)
        self.btnTop = tk.Button(
            self, command=self.topThreeClick, text="Display Top 3", height=2, width=14)
        self.btnDeleteBrand = tk.Button(
            self, command=self.deleteBrands, text="Delete Brand", height=2, width=14)
        self.updateRecord = tk.Button(
            self, command=self.updateProduct, text="Update Product", height=2, width=14)
        self.btnLeast = tk.Button(
            self, command=self.leastThreeBrands, text="Least 5 Brands", height=2, width=14)
    ...
    def categoryClick(self):
        ...
        self.cluster = MongoClient(
            "mongodb+srv://admin:adimn@cluster0.x5nnu.mongodb.net/Datatracker?retryWrites=true&w=majority")
